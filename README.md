...
- **Per-user ownership boundaries** (user-linked root directories, directory/file ownership checks).
- **Hierarchical folder model** with parent-child directories, breadcrumb path metadata, and recursive deletion behavior.
- **Session-based authentication** via signed `sid` cookies and Redis-backed session lookup.
- **Role-aware administration paths** (`user`, `manager`, `admin` model + middleware-protected user-management routes).
- **Modern React UI** (Vite + Tailwind) for file/folder operations with contextual actions.
- **Cloud upload path support** via AWS S3 presigned URL initiation endpoints.
